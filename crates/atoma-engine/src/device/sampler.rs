...
/// The sample of every selected row and the copy of its tokens back to the host.
pub struct Sample<'a> {
    call: SampleCall,
...
fn zeroed(stream: &Arc<CudaStream>, bytes: usize) -> Result<CudaSlice<u8>, SamplerError> {
    Ok(stream
