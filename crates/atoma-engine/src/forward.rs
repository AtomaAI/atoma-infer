...
pub trait Forward {
    ...
    /// Runs the model over `layout` and returns the logits of the rows the layout selected: one
    /// row per sampling entry, in batch order, a vocabulary wide.
    ///
    /// # Errors
    ///
    /// Returns the implementation's error when the step could not be run; the executor treats
    /// it as fatal.
    fn forward(&mut self, layout: &BatchLayout) -> Result<Logits<'_>, Self::Error>;
}
