...
//!
//! A request is one prompt, one set of sampling parameters, one priority and one egress sink. It
//! is born with one sequence and may hold more. Its phase — Waiting, Running, Preempted, Finished,
//! or Padding for a dummy — is a value only a legal transition can produce; prefilling and
//! decoding are derived from a sequence's computed count against its prompt length, never phases
//! of their own.
