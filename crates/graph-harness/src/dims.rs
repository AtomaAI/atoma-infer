//! Llama-8B-shaped model dimensions and the sizes the harness derives from them.
...
/// The model shape every harness step runs: Llama-8B-shaped per #143, with the layer
...
impl ModelDims {
    ...
    pub fn llama_8b_shaped(num_layers: usize) -> Self {
        assert!(
            ...
            "num_layers {num_layers} out of range: the harness runs 1..=32 layers"
        );
        let dims = Self {
            num_layers,
            hidden: 4096,
